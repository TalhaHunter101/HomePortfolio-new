...
export const searchHomepageDataWithHouse_Prices = async (query) => {
  try {
    ...
    const categorizedResults = {
      postcode: result2.aggregations.unique_postcodes.buckets.map(
        (bucket) => bucket.key
      ),
      townGroupedByCounty: result2.aggregations.towns_grouped_by_county.buckets.map(
        (bucket) => ({
          county: bucket.key,
    ...
    return processSearchResults(categorizedResults);
  } catch (error) {
    console.error("Elasticsearch query error:", error);
    throw new Error("Failed to fetch data from Elasticsearch");
  }
...
// Post-processing function to clean town names and filter out irrelevant entries
export const processSearchResults = (results) => {
  ...
  // Function to filter out irrelevant entries for London
  const isRelevantCountyForLondon = (county, town) => {
    const normalizedTown = normalizeTownName(town);
    // If it's 'London', it should primarily be in 'Greater London' or 'London'
    if (normalizedTown === "london") {
      return county.toLowerCase().includes("london");
    }
    return true; // For other towns, allow them
  };

  // Normalize and deduplicate postcodes
  const normalizedPostcodes = Array.from(
    new Set(results.postcode.map((postcode) => postcode.trim()))
  );

  // Process and normalize the town results
  const processedTownResults = results.townGroupedByCounty.map((entry) => {
    // Normalize and filter towns
    const normalizedTowns = Array.from(
      new Set(
        entry.towns
          .map(normalizeTownName) // Normalize town names
          .filter((town) => isRelevantCountyForLondon(entry.county, town)) // Filter out irrelevant towns
      )
    );
  ...
  return {
    postcode: normalizedPostcodes,
    townGroupedByCounty: processedTownResults,
  };
};
