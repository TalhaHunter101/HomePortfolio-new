...
export const searchHomepageDataWithHouse_Prices = async (query) => {
  try {
    // Prepare queries using more efficient match queries
    const esQueryForAddress = {
      multi_match: {
        query: query,
        fields: ['full_address^2', 'street_address', 'city', 'postcode'],
        type: 'phrase_prefix'
      }
    };
    ...
    const esQueryForPostCodeAndLocations = {
      multi_match: {
        query: query,
        fields: ['ref_postcode^2', 'location.townOrCity'],
        type: 'phrase_prefix'
      }
    };
    ...
    // Categorize results using Sets for uniqueness
    ...
    result2?.hits?.hits.forEach(hit => {
      const source = hit._source;
      if (source.ref_postcode) {
        categorizedResults.postcode.add(source.ref_postcode);
      }
      if (source.location?.townOrCity) {
        categorizedResults.posttown.add(source.location.townOrCity);
      }
    });
    ...
    // Process hits without individual async calls
    hits.forEach(hit => {
      const source = hit._source;
    ...
    categorizedResults.postcode = Array.from(categorizedResults.postcode);
    categorizedResults.posttown = Array.from(categorizedResults.posttown);
