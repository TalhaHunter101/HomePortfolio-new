...
export const getAllDemoGraphicDataByPostcode = async (postcode) => {
  try {
    const result = await client.search({
    ...
    const demographic = await client.search({
      index: "walkscore",
      body: {
        query: {
          term: {
    ...
    const demographicsHits = result?.hits?.hits;
    const walkscoreHits = demographic?.hits?.hits;

    return {
      demographics: demographicsHits,
      walkscore: walkscoreHits,
    };
  } catch (error) {
    console.error("Elasticsearch search error:", error);
    throw error;
