...
const amenities = [
    { name: 'Restaurants', icon: 'ph:chef-hat-duotone', color: 'blue', key: 'restaurant',bg_color:'bg-blue-400' },
    { name: 'Coffee', icon: 'line-md:coffee-half-empty-filled-loop', color: 'brown', key: 'cafe',bg_color:'bg-brown-400' },
    { name: 'Bars', icon: 'fluent:drink-beer-24-regular', color: 'goldenrod', key: 'bar',bg_color:'bg-desert-200' },
    { name: 'pubs', icon: 'fluent:drink-beer-24-regular', color: 'goldenrod', key: 'pub',bg_color:'bg-desert-200' },
    { name: "Parking", icon: 'fluent-mdl2:parking-solid', color: 'gray', key: 'parking',bg_color:'bg-gray-400' },
    { name: 'Groceries', icon: 'material-symbols:grocery-sharp', color: 'purple', key: 'supermarket',bg_color:'bg-purple-400' },
    { name: 'Parks', icon: 'ic:twotone-park', color: 'green', key: 'park',bg_color:'bg-green-400' },
    { name: 'Hospitals', icon: 'noto-v1:hospital', key: 'hospital',bg_color:'bg-red-400' },
    { name: 'Fast Food', icon: 'noto-v1:hamburger', key: 'fast_food',bg_color:'bg-yellow-400' },
    { name: 'nursery', icon: 'cbi:roomsnursery', color: 'purple', key: 'kindergarten',bg_color:'bg-purple-400' },
]
...
const WhatsNearbyMap = ({ center }) => {
    ...
    async function getNearbyLocations(lat, lon, amenity = selectedAmenity, radius = 5000, limit = 10) {
            ...
            <div className="absolute top-4 left-4 right-4 flex space-x-2 overflow-x-auto z-10 scrollbar-hide">
                {amenities.map((amenity) => (
                    <Button
                        ...
                        startContent={<Icon icon={amenity.icon} height={48} width={"24px"} color={amenity.color || 'black'} />}
                    >
                        {amenity.name}
                    </Button>
                ))}
                ...
                {locations?.map((place) => (
                 place.name !== "na" ?  <PlaceCard key={place?.id} place={place} /> : null
                ))}
            </div>
            <div className="absolute inset-0 z-0">
        <NearByPlacesStatic center={center} height="500px" locations={locations} amenities={amenities} />
      </div>
        </div>
    );
};
        ...
        return (
            ...
            <div className="rounded-lg py-lg card flex flex-col h-full relative border-gray-150 bg-gray-100 sm:rounded-lg border">
                ...
                <div className="flex items-center">
                    ...
                    <li className="text-gray-800 text-md font-semibold line-clamp-1">{place.name}</li>
